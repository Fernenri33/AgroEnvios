<script>
    ...
    import { checkAuthentication, fetchEnvios } from '$lib/misEnvios';
    ...
    onMount(async () => {
        ...
        try {
            ...
            // Filtra solo los envíos aceptados
            envios = result.envios.filter(e => e.estado === 'En_Revision');
...
<div class="flex h-screen">
    ...
    <main class="flex-1 bg-gray-100 p-6">
        <div class="flex items-center justify-between mb-4">
            <h1 class="text-2xl font-bold">Lista de Envios Aceptados</h1>
        </div>

        {#if error}
            <p class="text-red-500">{error}</p>
        {:else if envios.length === 0}
            <p class="text-gray-700">{mensaje || 'No hay envíos aceptados.'}</p>
        {:else}
